/********************************************************
 * Project VltgD - Yet Another IoT power strick
 * Powered by Dedsic
 * Fork from https://t.co/qE4VegoF00
 * See user guide on wiki page
 * Ver 1.0
********************************************************/
//#include <Arduino.h>
#include <FS.h>
...
// Defines
#define DefaultName "VltgD" // Default device name
...
// NTP Server details
//-------------------
IPAddress timeServer(118, 140, 184, 99);
WiFiUDP Udp;
unsigned int localPort = 1919; // Local UDP port
WiFiServer server(80);

//define your default values here, if there are different values in config.json, they are overwritten.
//length should be max size + 1
char static_ip[16] = "192.168.255.128";
char static_gw[16] = "192.168.255.1";
...
// IO config
#define Relay 5  // (D1) Control signal
#define LED 2    // (D4) Satatus LED
...
void ReadData()
{
    // EEPROM location 0 will contain EEPROM_chk if data is valid. If not, store defult settings
    if (EEPROM.read(0) != EEPROM_chk)
    {
        ...
        EEPROM.write(8, 118);        // NTP IP 1
        EEPROM.write(9, 140);          // NTP IP 2
        EEPROM.write(10, 184);        // NTP IP 3
        EEPROM.write(11, 99);        // NTP IP 4
        // Clear all programs
...
void sendNTPpacket(IPAddress &address)
{
    ...
    // 8 bytes of zero for root delay/dispersion
    packetBuffer[12] = 49;
    packetBuffer[13] = 0x4E;
    packetBuffer[14] = 49;
    packetBuffer[15] = 52;
    // all NTP fields have been valued, now send a packet to port 123 requesting a timestamp
...
{
    old_ms = millis();
    // 10s delay to allow user to configure Network
...
void DoTimeCheck()
{
    ...
    if (Mode == 1)
    {
        ...
        // clear button overrides
        ManualOff = false;
        ManualOn = false;      //Manually push switch button
        ManualTime = false;
        return;
    }
    // Is mode invalid
...
void setup()
{
    ...
    digitalWrite(Relay, LOW);
    digitalWrite(LED, LOW);
    CheckReset();    // Reset WiFi connection if button pressed
...
void loop()
{
    ScanButton();
    UpdateLED();
    CheckNTPtime();     // Scan for NTP time changes
    DoTimeCheck();    // See if time has changed
    WiFiClient client = server.available();    // Handle Web page
    request = client.readStringUntil('\r');    // Read requests from web page
    client.flush();
    // See if data was received
    if (request.indexOf("/") != -1)
    {
        //Serial.println(request);
        if (request.indexOf("Link=1") != -1)
            Page = 1;
        if (request.indexOf("Link=2") != -1)
            Page = 2;
        if (request.indexOf("Link=3") != -1)
            Page = 3;
        if (request.indexOf("Link=4") != -1)
            Page = 4;
        if (request.indexOf("Link=5") != -1)
            Page = 5;
        ...
        if (request.indexOf("SaveBtn3=") != -1)
        {
            ...
            if (request.indexOf("mode=Auto") != -1)
                Mode = 2;
            if (request.indexOf("mode=On") != -1)
                Mode = 1;
            if (request.indexOf("mode=Off") != -1)
                Mode = 0;
            //get button when on
            if (request.indexOf("BtnOff=0") != -1)
                OffMode = 0;
            if (request.indexOf("BtnOff=1") != -1)
                OffMode = 1;
            // get button when off
            if (request.indexOf("BtnOn=0") != -1)
                OnMode = 0;
            if (request.indexOf("BtnOn=1") != -1)
                OnMode = 1;
            if (request.indexOf("BtnOn=2") != -1)
                OnMode = 2;
            // get timer on hour
        ...
        client.println("<head>");
        client.println("<META HTTP-EQUIV=""refresh"" CONTENT=""10"">");     // Refresh home page every 10 sec
        client.print("<style>html,body {background-color: #e6e6e6;Color: #669999;}</style>");
        client.println("<title>");
        client.println("VoltgD");
        client.println("</title>");
        client.println("</head>");
        client.print("<body>");
        //Show time
        ...
        client.print("<p style=\"color:#ffad33;\";>");
        client.print("<font size = \"5\"><b>");
        if (hour() < 10)
            client.print(" ");
        client.print(hour());
        ...
        client.println("Current Power: </b>");
        client.print(volt );
        client.print("V");
        client.print( crnt );
        client.print("A");
        ...
        if (Page == 1)
        {
            ...
            client.print("<br>");
            client.print("&copy; ");
            client.print("<a href=\"https://vectordigi.tk\">Vector Di-gi</a>");
            client.print(" All Rights Reserved. ");
            client.print("</font>");
        }
        ...
        if (Page == 3)
        {
            client.print("<font size = \"4\"><b>Configurations</font></b><br><br>");
            // Device Name
        ...
        if (Page == 4)
        {
            client.print("<font size = \"4\"><b>Automatic Correcting Time</font></b><br><br>");
            //Time Zone
            client.print("<font size = \"3\"><b>NTP settings</font></b><br><br>");
            ...
            client.print("NTP Server IP (118.140.184.99 by default) : <br>");
            ...
            // Button 1
            client.println("<br><br>");
            client.println("<input type=\"submit\" name =\"SaveBtn4\" value=\"Sync\"><br>");
            // Draw line
            client.print("<hr />");
            // Set Time Inputs
            client.print("<font size = \"3\"><b>Manual Adjust</font></b><br>");
            ...
            //Button 2
            client.println("<br><br>");
            client.println("<input type=\"submit\" name =\"SaveBtn5\" value=\"Applicate\">");
            //Button 3
            client.println("&emsp;");
            client.println("<input type=\"submit\" name =\"RefreshBtn\" value=\"Refresh\">");
            client.print("<hr />");            // Draw line
            // Show last time synch
            client.print("<font size = \"2\">");
            client.print("</b>Latest update time : ");
            client.print(Lastdd);
            client.print("/");
            if (Lastmm < 10)
                client.print("0");
